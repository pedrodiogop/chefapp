  ...
  FutureEither<UserModel> signInWithGoogle() async {
    try {
      final GoogleSignInAccount? googleUser = await _googleSignIn
          .signIn(); // para o utilizador escolher a conta de autenticação
      ...
      final googleAuth = await googleUser
          ?.authentication; // estamos obtendo a informação da autenticação do usuario
      ...
      UserCredential userCredential = await _auth.signInWithCredential(
          credential); // esta linha de codigo serve para registar ou fazer login de umutlizador
      ...
      if (userCredential.additionalUserInfo!.isNewUser) {
        // FOI FEITO POR CAUSA DO KARMA
        userModel = UserModel(
            // dar valores as variaveis para serem guardadas na base de dados
            ...
            aseguir: [],
            seguidores: [],
            dataUltimaReceita: null);
        await _users
            .doc(userCredential.user!.uid)
            .set(userModel.toMap()); // guardar as informações na coleçao
      } else {
